...
/// <summary>
/// Morningstar DBRS provider-ratings source — <b>pending real API</b>. The class, its options
/// (<see cref="PrismOptions.ProviderApiOptions.MorningstarApi"/>) and its DI registration exist so the
...
/// </summary>
public sealed class MorningstarDbrsRatingsClient(
    IOptions<PrismOptions> options,
    ILogger<MorningstarDbrsRatingsClient> logger) : IProviderRatingsSource
{
    public Provider Provider => Provider.MorningstarDbrs;
    ...
    public Task<ProviderRatingRecord?> GetRatingAsOfAsync(string issuerId, DateTimeOffset asOf, CancellationToken ct)
    {
        ...
        logger.LogWarning(
            "Morningstar/DBRS provider API integration is pending (configured={Configured}); returning no coverage for {IssuerId}.",
            options.Value.ProviderApis.MorningstarApi is not null,
            issuerId);
